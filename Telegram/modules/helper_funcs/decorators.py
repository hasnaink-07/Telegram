from Telegram.modules.disable import (
...
class AnieTelegramHandler:
    ...
    def command(
        ...
        def _command(func):
            try:
                ...
                log.debug(
                    f"[TELEGRAM] Loaded handler {command} for function {func.__name__} in group {group}"
                )
            except TypeError:
                if can_disable:
                    self._dispatcher.add_handler(
                ...
                log.debug(
                    f"[TELEGRAM] Loaded handler {command} for function {func.__name__}"
                )
    ...
    def message(
        ...
        def _message(func):
            try:
                ...
                log.debug(
                    f"[TELEGRAM] Loaded filter pattern {pattern} for function {func.__name__} in group {group}"
                )
            except TypeError:
                if can_disable:
                    self._dispatcher.add_handler(
                ...
                log.debug(
                    f"[TELEGRAM] Loaded filter pattern {pattern} for function {func.__name__}"
                )
    ...
    def callbackquery(self, pattern: str = None, run_async: bool = True):
        def _callbackquery(func):
            ...
            log.debug(
                f"[TELEGRAM] Loaded callbackquery handler with pattern {pattern} for function {func.__name__}"
            )
            return func
